$(document).ready(function () {
  ...
  function formatCurrencyHtml(number) {
    const amount = Number(number);
    const safeAmount = Number.isFinite(amount) ? amount : 0;
    const formattedAmount = currencyFormatter 
      ...
      : (typeof safeAmount.toLocaleString === "function" 
          ? safeAmount.toLocaleString("vi-VN") 
          : String(safeAmount));
    return `<span class="price-amount">${formattedAmount}</span><span class="price-currency">đ</span>`;
  }
  ...
  function applyServerTotals(cartDetailId, payload) {
    ...
    const badgeCount = payload.cartItemCount != null ? payload.cartItemCount : payload.cartCount;
    if (
      badgeCount != null &&
      window.CartCommon &&
      typeof window.CartCommon.updateCartBadge === "function"
    ) {
      window.CartCommon.updateCartBadge(badgeCount);
    }
  }
  ...
  function updateTotal() {
    ...
    if (cartInputs.length) {
      ...
      checkoutItems.each(function () {
        ...
        const qty =
          parseInt(
            qtyEl.is("input, select, textarea") ? qtyEl.val() : qtyEl.text(),
            10
          ) || 0;
    ...
    const totalPriceEl = $("[data-cart-total-price]");
    if (totalPriceEl.closest('.checkout-summary-total').length) {
      totalPriceEl.html(formatCurrencyHtml(total));
    } else {
      totalPriceEl.text(formatCurrency(total));
    }
  ...
  function updateCheckoutQuantityDisplay(cartDetailId, qty) {
    ...
    if (quantityEl.length) {
      if (quantityEl.is("input, select, textarea")) {
        quantityEl.val(qty);
      } else {
        quantityEl.text(qty);
      }
  ...
  function refreshCartDisplayFromCurrentValues() {
    ...
    if (visibleInputs.length) {
      ...
      checkoutItems.each(function () {
        ...
        const qtyRaw = qtyEl.is("input, select, textarea") ? qtyEl.val() : qtyEl.text();
  ...
  $(".btn-plus")
    ...
    .on("click", function () {
      ...
      const maxStock = parseInt(input.data("stock"), 10) || 999;
      
      if (currentValue >= maxStock) {
        if (typeof window.showCustomToast === "function") {
          window.showCustomToast("Thông báo", `Số lượng không được vượt quá ${maxStock} sản phẩm`, "warning");
        }
        return;
      }
  ...
  $(CART_QUANTITY_INPUT_SELECTOR)
    ...
    .on("change.cartQuantity blur.cartQuantity", function () {
      const input = $(this);
      let nextValue = sanitizeQuantity(input.val());
      const maxStock = parseInt(input.data("stock"), 10) || 999;
      
      if (nextValue > maxStock) {
        nextValue = maxStock;
        if (typeof window.showCustomToast === "function") {
          window.showCustomToast("Thông báo", `Số lượng đã được điều chỉnh về ${maxStock} (tối đa)`, "warning");
        }
      }
