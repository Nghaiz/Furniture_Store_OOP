  ...
  function initMiniCart() {
    ...
    function normalizePayload(data) {
      ...
      if (data && Array.isArray(data.items)) {
        const cartItemCount = typeof data.cartItemCount === "number" && Number.isFinite(data.cartItemCount)
            ? data.cartItemCount
            : data.items.length;
        const totalQuantity = typeof data.totalQuantity === "number" && Number.isFinite(data.totalQuantity)
            ? data.totalQuantity
      ...
      return { items: [], cartItemCount: 0, totalQuantity: 0 };
    }

    function renderItems(items, totalQuantity, cartItemCount) {
      if (!Array.isArray(items) || !items.length) {
        setState("Chưa có sản phẩm trong giỏ hàng", "empty");
        return;
      }
      
      const distinctCount = Number.isFinite(cartItemCount) && cartItemCount > 0 
        ? cartItemCount 
        : items.length;
      ...
      updateCartBadgeValue(distinctCount);
    }
    ...
    function fetchMiniCart() {
      ...
      fetch(fetchUrl, {
        ...
        .then((data) => {
          ...
          renderItems(payload.items, payload.totalQuantity, payload.cartItemCount);
        })
        .catch(() => {
